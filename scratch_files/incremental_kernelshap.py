...
class incrementalSHAP:
    def __init__(self, M, y_fullset, y_emptyset):
        ...
        #Lookup table
        self.weight_vector = np.zeros(M+1)
        for k in range(M+1):
            self.weight_vector[k] = self.shapley_kernel(k)

        #Infinite weight constant
        self.infinite_weight = 10000
        self.counter_full_max = 0
    ...
    def generate_k_subsets(self,k, n_samples=10, balanced=True,duplicates=False):
        counter_samples = 0
        ...
        while 1 == 1:
            ...
            if S_sample_tuple not in S_used or duplicates:
                ...
                if counter_samples == n_samples_cap:
                    break
        return S
    def kernel_shap(self,S,S_pred,prediction,offset):
        tmp = np.concatenate((np.zeros((1, self.M)), S, np.ones((1, self.M))))
        S_bar = np.zeros((np.shape(tmp)[0], self.M + 1))
        S_bar[:, -1] = 1
        S_bar[:, :-1] = tmp
        weights_bar = np.zeros((np.shape(S_bar)[0]))
        S_bar_sizes = np.sum(S_bar,-1)-1
        for i,size in enumerate(S_bar_sizes):
            weights_bar[i] = self.weight_vector[int(size)]
        S_bar_pred = np.concatenate((offset, S_pred, prediction))
        tmp = np.linalg.inv(np.dot(np.dot(S_bar.T, np.diag(weights_bar)), S_bar))
        kernel_shap_estimate = np.dot(tmp, np.dot(np.dot(S_bar.T, np.diag(weights_bar)), S_bar_pred))
        return kernel_shap_estimate

    def create_subsets_from_permutation(self,permutation):
        S = np.zeros((self.M-2,self.M))
        for pos in range(1,self.M):
            S[pos-1,permutation[:pos]]


    def generate_subsets(self,budget,balanced=True,duplicates=False):
        ...
        counter = 0
        self.full_subset_flag = np.zeros(np.sum(weighted_sample_sizes),dtype=bool)

        for k in range(1,np.int(self.M/2)+1):
            number_of_samples = np.int(weighted_sample_sizes[k-1])
            if  number_of_samples >= scipy.special.binom(self.M,k):
                S_k = S_full[S_full_sizes==k,:]
                S_k_inv = 1-S_k
                self.full_subset_flag[counter:counter+2*np.shape(S_k)[0]] = 1
                print("full",k)
            else:
                if k == self.M / 2:
                    number_of_samples_half = int(number_of_samples/2)
                    S_k = self.generate_k_subsets(k, number_of_samples_half, balanced,duplicates=duplicates)
                else:
                    S_k = self.generate_k_subsets(k,number_of_samples,balanced,duplicates=duplicates)
                S_k_inv = 1-S_k
                print("sample",k)
            S_k_combined = np.concatenate((S_k,S_k_inv))
            S_final[counter:counter+np.shape(S_k_combined)[0],:] = S_k_combined
            counter += np.shape(S_k_combined)[0]

            print(np.shape(S_k),np.shape(S_k_inv))
    ...
    def evaluate_subsets(self,S_shap):
        mu1_Q = np.zeros(self.M)
        mu2_Q = np.zeros((self.M, self.M))
        for q in S_shap:
            size = np.sum(q)
            kernel = self.weight_vector[int(size)]
            for i in range(M):
                if q[i]:
                    mu1_Q[i] += kernel
...
if __name__ == "__main__":
    ...
    S = inc_shap.generate_subsets(1500,balanced=False,duplicates=True)
    S_pred = inc_shap.generate_predictions(S,f,x_explain,reference)


    range_indices = np.arange(0,np.shape(S)[0])
    range_indices_sampled = range_indices[np.invert(inc_shap.full_subset_flag)]
    range_indices_sampled_fix = range_indices[np.invert(inc_shap.full_subset_flag)]
    np.random.shuffle(range_indices_sampled)
    S_perm = S.copy()
    S_perm[range_indices_sampled_fix] = S[range_indices_sampled,:]
    S_perm_pred = S_pred.copy()
    S_perm_pred[range_indices_sampled_fix] = S_pred[range_indices_sampled]

    shap_est = []
    #Incremental SHAP
    for s,s_pred in zip(S_perm,S_perm_pred):
        inc_shap.update_shap(s,s_pred)
        shap_est.append(inc_shap.shapley_estimate())




    #KernelSHAP
    kernel_shap_estimate = inc_shap.kernel_shap(S,S_pred,prediction,offset)

    n_points = len(shap_est[150:])
    plot_range = np.arange(0,len(shap_est[150:]))
    import matplotlib.pyplot as plt

    test= []
    for i in range(n_points):
        test.append(kernel_shap_estimate[:-1])

    plt.figure()
    plt.plot(shap_est[150:])
    plt.plot(test,ls='--')
    plt.show()
    print("DIFF Inc-SHAP:", np.sum(np.abs(shap_est[-1] - reg_shap[:-1])))
    print("DIFF KernelSHAP:", np.sum(np.abs(kernel_shap_estimate[:-1] - reg_shap[:-1])))


    Q1, Q2 = inc_shap.evaluate_subsets(S)
