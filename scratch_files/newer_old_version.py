...
def compute_shap_values(f,S_shap,y_shap,M,x,reference):
    ...
    mu1_Q_val = np.mean(mu1_Q)
    mu2_Q_val = np.sum(mu2_Q)/(M*(M-1))
    print(mu1_Q_val,mu1_est)
    print(mu2_Q_val,mu2_est)
    print(mu1_Q_val-mu2_Q_val,mu_est)
...
my_shap,old_shap = compute_shap_values(f,explainer.maskMatrix,explainer.y,explainer.M,x,explainer.data.data)
#my_shap,old_shap = compute_shap_values(f,S[:,:-1],np.expand_dims(y,1),explainer.M,x,explainer.data.data)


print("New SHAP Result:", my_shap.T, np.sum(my_shap))
print("DIFF:", np.sum(np.abs(my_shap.T - reg_shap[:-1])))

print("Old SHAP Result:", old_shap.T, np.sum(old_shap))
print("DIFF:", np.sum(np.abs(old_shap[:-1,0] - reg_shap[:-1])))