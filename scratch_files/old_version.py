...
def kernel_shap_k(f,x,reference,M,k_approx=3,batch_size=256):
    y_emptyset = f(np.zeros((1, M)) + reference)
    y_fullset = f(x.reshape(1, -1))
    ...
    empty_row[-1,-1]  = 1
    full_row = np.ones((1, M+1), dtype=bool)
    S_bar = np.concatenate((empty_row,S, full_row)) # S_bar empty set und allen 1er
    S_bar_unique = np.concatenate((empty_row,S_unique, full_row))

    reg_weights_bar = np.concatenate(([10000], reg_weights, [10000])) # alle 0 und alle 1
    reg_weights_bar_unique = np.concatenate(([10000000], reg_weights_unique, [10000000]))
    y_bar = np.concatenate((y_emptyset,y,y_fullset))
    y_bar_unique = np.concatenate((y_emptyset, y_unique,y_fullset))

    # Empritrische werte der gewichte ... was erwartet man das ist das mu_1 und u_2 im paper der theo wert
    mu1_Q = np.zeros(M)  # TODO (4) links summe der weights nur von i
    mu2_Q = np.zeros((M, M))  # TODO (4) rects summe der weights mit i und j
    ...
    # (y_fullset-y_emptyset)/M = C/D
    # mu_est = mu
    # S letzte spalte wird rausgenommen weil das ist eine 1er spalte die bias in der regression braucht man nur dort nicht bei new_shap
    # np.dot(S[:,:-1].T,np.diag(reg_weights) alle mit i
    #
